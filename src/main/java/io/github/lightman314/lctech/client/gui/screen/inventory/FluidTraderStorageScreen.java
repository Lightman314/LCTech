...
public class FluidTraderStorageScreen extends ContainerScreen<FluidTraderStorageContainer>{
	...
	public static void drawTraderStorageBackground(MatrixStack matrix, Screen screen, FontRenderer font, Container container, Minecraft minecraft, int xSize, int ySize, IFluidTrader trader)
	{
		...
		screen.blit(matrix, startX + FluidTraderUtil.getInventoryDisplayOffset(trader), startY + FluidTraderUtil.getTradeDisplayHeight(trader), 0, 111, 176 + 32, 100);
		
		//Draw the fake fluid trade buttons
		for(int i = 0; i < trader.getTradeCount(); i++)
		{
			FluidTradeButton.renderFluidTradeButton(matrix, screen, font, startX + FluidTraderUtil.getButtonPosX(trader, i), startY + FluidTraderUtil.getButtonPosY(trader, i), i, trader, null, false, true, true);
		}
	...
	public static void drawTraderStorageForeground(MatrixStack matrix, FontRenderer font, IFluidTrader trader, int ySize, ITextComponent title, ITextComponent inventoryTitle)
	{
		...
		font.drawString(matrix, title.getString(), 8.0f + FluidTraderUtil.getTradeDisplayOffset(trader), 6.0f, 0x404040);
		
		font.drawString(matrix, inventoryTitle.getString(), FluidTraderUtil.getInventoryDisplayOffset(trader) + 8.0f, ySize - 94, 0x404040);
	...
	protected void init()
	{
		...
		int traderOffset = FluidTraderUtil.getTradeDisplayOffset(this.container.tileEntity);
		int inventoryOffset = FluidTraderUtil.getInventoryDisplayOffset(this.container.tileEntity);
		...
		{
			this.tradePriceButtons.add(this.addButton(new PlainButton(this.guiLeft + FluidTraderUtil.getPriceButtonPosX(this.container.tileEntity, i), this.guiTop + FluidTraderUtil.getPriceButtonPosY(this.container.tileEntity, i), 10, 10, this::PressTradePriceButton, GUI_TEXTURE, 176 + 64, 40)));
		}
	...
	public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks)
	{
		...
		else
		{
			...
			{
				int result = FluidTradeButton.tryRenderTooltip(matrixStack, this, i, tileEntity, this.guiLeft + FluidTraderUtil.getButtonPosX(tileEntity, i), this.guiTop + FluidTraderUtil.getButtonPosY(tileEntity, i), mouseX, mouseY, null, true);
	...
	public boolean mouseClicked(double mouseX, double mouseY, int button)
	{
		...
		for(int i = 0; i < tradeCount; ++i)
		{
			...
			int buttonX = this.guiLeft + FluidTraderUtil.getButtonPosX(this.container.tileEntity, i);
