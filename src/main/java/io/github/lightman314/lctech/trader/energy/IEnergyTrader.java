...
public interface IEnergyTrader extends ITrader, IUpgradeable, ITradeRuleHandler, ILoggerSupport<EnergyShopLogger> {
	...
	public static List<ITextComponent> getEnergyHoverTooltip(IEnergyTrader trader)
	{
		List<ITextComponent> tooltip = Lists.newArrayList();
		tooltip.add(new StringTextComponent(EnergyUtil.formatEnergyAmount(trader.getTotalEnergy()) + "/" + EnergyUtil.formatEnergyAmount(trader.getMaxEnergy())).mergeStyle(TextFormatting.AQUA));
		if(trader.getPendingDrain() > 0)
		{
			tooltip.add(new TranslationTextComponent("gui.lctech.energytrade.pending_drain", EnergyUtil.formatEnergyAmount(trader.getPendingDrain())).mergeStyle(TextFormatting.AQUA));
		}
		return tooltip;
	}
